...
public class PlayerMovement : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!IsDashing && !IsJumping)
		{
			...
			if (Physics2D.OverlapBox(_groundCheckPoint.position, _groundCheckSize, 0, _groundLayer)) //checks if set box overlaps with ground
			{
				...
				LastOnGroundTime = movementData.coyoteTime; //if so sets the lastGrounded to coyoteTime
            }		
		...
		if (IsWallJumping && Time.time - _wallJumpStartTime > movementData.wallJumpTime)
		{
			IsWallJumping = false;
		}
		...
		if (CanDash() && LastPressedDashTime > 0)
		{
			...
			Sleep(movementData.dashSleepTime); 
		...
		if (!_isDashAttacking)
		{
			...
			else if ((IsJumping || IsWallJumping || _isJumpFalling) && Mathf.Abs(RB.velocity.y) < movementData.jumpHangTimeThreshold)
			{
    ...
    private void FixedUpdate()
	{
		...
		{
			Run(movementData.dashEndRunLerp);
		}
	...
	{
		LastPressedDashTime = movementData.dashInputBufferTime;
	}
    #endregion
    ...
    private void Run(float lerpAmount)
	{
		...
		float targetSpeed = _moveInput.x * movementData.runMaxSpeed;
		...
		if(movementData.doConserveMomentum && Mathf.Abs(RB.velocity.x) > Mathf.Abs(targetSpeed) && Mathf.Sign(RB.velocity.x) == Mathf.Sign(targetSpeed) && Mathf.Abs(targetSpeed) > 0.01f && LastOnGroundTime < 0)
		{
	...
	private IEnumerator StartDash(Vector2 dir)
	{
		...
		while (Time.time - startTime <= movementData.dashAttackTime)
		{
			RB.velocity = dir.normalized * movementData.dashSpeed;
		...
		SetGravityScale(movementData.gravityScale);
		RB.velocity = movementData.dashEndSpeed * dir.normalized;

		while (Time.time - startTime <= movementData.dashEndTime)
		{
			yield return null;
		}
	...
	private IEnumerator RefillDash(int amount)
	{
		...
		_dashesLeft = Mathf.Min(movementData.dashAmount, _dashesLeft + 1);
	}
	#endregion
	...
	private void Slide()
	{
		...
		float speedDif = movementData.slideSpeed - RB.velocity.y;	
	...
	{
		if (!IsDashing && _dashesLeft < movementData.dashAmount && LastOnGroundTime > 0 && !_dashRefilling)
		{
